...
export default function PaymentConfirmation() {
  ...
  useEffect(() => { if (!sessionId) { setState('error'); setMessage('The checkout confirmation is missing.'); return; } confirmPayment(sessionId).then(() => { setState('success'); setMessage('Payment confirmed and your place is booked.'); }).catch((error) => { setState('error'); setMessage(error instanceof Error ? error.message : 'We could not confirm the booking.'); }); }, [sessionId]);
